...
class EditableItem(BaseItem):
    ...
    def __init__(self, scene, program: GL.Program, value: str = '', pos: list[float] = [0.0, 0.0, 0.0], name=''):
        super().__init__(scene, name)
        self.setPos(pos)
        self.setColor(hexToRGB('#c800ff'))
        self._value = 0.0
        self._input_type = EditableItem.InputTypeFloat
        ...
        if self._value:
            ...
            for polygon in polygons:
                for point in polygon:
                    vertices.append(point.x() + self.x())
                    vertices.append(-point.y() + self.y())
    ...
    def startEditing(self):
        dialog = EditValueDialog(self._input_type)
        dialog.exec()
